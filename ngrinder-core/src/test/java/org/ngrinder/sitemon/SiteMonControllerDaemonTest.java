...
public class SiteMonControllerDaemonTest {
	...
	SiteMonController controller;
	SiteMonControllerDaemon daemon;
	
	@Before
	public void before() {
		controller = mock(SiteMonController.class);
		daemon = new SiteMonControllerDaemon(controller);
	}
	...
	public void testRetry() throws Exception {		
		...
		for (int retry = 2; retry < 5; retry++) {
			ThreadUtils.sleep(SiteMonControllerDaemon.CONTROLLER_RETRY_INTERVAL);
