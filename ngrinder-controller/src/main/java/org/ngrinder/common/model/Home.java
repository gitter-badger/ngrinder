...
public class Home {
	...
	private static final String PATH_SITE_MON = "sitemon";
	...
	/**
	 * Create temp directory for SiteMonDist.
	 * Delete the directory after use.
	 * 
	 * @return 
	 */
	public SiteMonDistDirectory createTempSiteMonDistDirectory(String siteMonId) {
		SiteMonDistDirectory tmpDist = new SiteMonDistDirectory();
		File dist = new File(getSiteMonDirectory(), System.nanoTime() + "");
		File scriptDist = new File(dist, siteMonId);
		tmpDist.setRootDirectory(mkDir(dist));
		tmpDist.setScriptDirectory(mkDir(scriptDist));
		return tmpDist;
	}
	
	private File getSiteMonDirectory() {
		return getSubFile(PATH_SITE_MON);
	}
